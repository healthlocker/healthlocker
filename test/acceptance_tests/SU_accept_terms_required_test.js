...
// Homepage > Sign up > pw and security q > do not accept terms of service - should receive 'must be accepted' error message
...
nightmare
  ...
  .wait(1000)
  .screenshot('./test/acceptance_tests/screenshots/SU3_terms_not_accepted.png')
  .evaluate(function(){
    return document.getElementsByTagName('span')[0].innerHTML;
  })
  .end()
    .then(function (result) {
      result == `must be accepted` ? console.log(`PASS: error message given for failing to accept terms of service`) : console.log(`FAIL: did not correctly give error message for not agreeing to the terms of service`);
    })
